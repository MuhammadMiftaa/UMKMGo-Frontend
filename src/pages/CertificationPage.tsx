...
export function CertificationPage() {
  ...
  const certificationApplications = mockApplications.filter((app) => app.type === "sertifikasi")

  const filteredApplications = certificationApplications.filter((app) => {
    const matchesSearch =
  ...
  const getSLABadge = (deadline: string) => {
    ...
    const hoursLeft = Math.max(0, (slaDate.getTime() - now.getTime()) / (1000 * 60 * 60))
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Sertifikasi UMKM</h1>
        <p className="text-muted-foreground">Kelola pengajuan sertifikasi untuk UMKM</p>
      </div>
              ...
              <div className="flex items-start justify-between">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    <span>
                      <strong>Skema:</strong> {application.certificationScheme}
                    </span>
                    <span>
      ...
      {filteredApplications.length === 0 && (
        <Card>
          <CardContent className="p-6 text-center">
            <p className="text-muted-foreground">Tidak ada pengajuan sertifikasi yang ditemukan.</p>
          </CardContent>
        </Card>
      )}
    </div>
