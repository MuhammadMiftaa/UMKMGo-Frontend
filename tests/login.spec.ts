...
test.describe('Login Page', () => {
  ...
  test('should display login form elements', async ({ page }) => {
    ...
    await expect(logo).toHaveAttribute('src', 'https://res.cloudinary.com/dblibr1t2/image/upload/v1762742634/umkmgo_logo.png');
  ...
  test('should successfully login with valid credentials', async ({ page }) => {
    // Mock API response untuk login sukses
    await page.route('**/api/login', async (route) => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          token: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InVta20xQHVta20uZ28uaWQiLCJleHAiOjE3NjU3NTUyMjAsImlhdCI6MTc2NTQ5NjAyMCwiaWQiOjEwMSwiaXNfYWRtaW4iOnRydWUsIm5hbWUiOiJVTUtNIFVzZXIgMSIsInBlcm1pc3Npb25zIjpbIlZJRVdfVFJBSU5JTkciXSwicm9sZSI6NCwicm9sZV9uYW1lIjoicGVsYWt1X3VzYWhhIn0.MIusbQkW_6479_1Pf1gad49SFgtjI1UT9KeueZDRbaw'
        })
      });
    });
  ...
  test('should redirect to home if user already logged in', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      ...
      localStorage.setItem('token', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InVta20xQHVta20uZ28uaWQiLCJleHAiOjE3NjU3NTUyMjAsImlhdCI6MTc2NTQ5NjAyMCwiaWQiOjEwMSwiaXNfYWRtaW4iOnRydWUsIm5hbWUiOiJVTUtNIFVzZXIgMSIsInBlcm1pc3Npb25zIjpbIlZJRVdfVFJBSU5JTkciXSwicm9sZSI6NCwicm9sZV9uYW1lIjoicGVsYWt1X3VzYWhhIn0.MIusbQkW_6479_1Pf1gad49SFgtjI1UT9KeueZDRbaw');
    });
  ...
  test('should clear error message when resubmitting form', async ({ page }) => {
    // Submit dengan kredensial salah
    await page.getByLabel('Email').fill('wrong@email.com');
    ...
    await page.getByLabel('Password').fill('another');
    await page.getByRole('button', { name: 'Masuk' }).click();

    // Error seharusnya hilang sementara (sebelum muncul error baru)
    // Ini menguji bahwa setError("") dipanggil
  });
